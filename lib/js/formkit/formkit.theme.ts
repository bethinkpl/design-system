...
/**
  ...
  * ```
  **/
 export function rootClasses (sectionKey: string, node: FormKitNode<unknown>): Record<string, boolean> {
   const key = `${node.props.type}__${sectionKey}`
   const semanticKey = `formkit-${sectionKey}`
   const familyKey = node.props.family ? `family:${node.props.family}__${sectionKey}` : ''
   const memoKey = `${key}__${familyKey}`
   if (!(memoKey in classes)) {
     const sectionClasses = classes[key] ?? globals[sectionKey] ?? {}
