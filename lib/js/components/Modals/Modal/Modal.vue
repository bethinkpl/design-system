<template>
  <div class="modal"
    ...
    <div class="modal__wrapper" :class="{ '-small': size === MODAL_SIZES.SMALL, }">
      <div v-if="headerImage" class="modal__image" v-bind:style="{ backgroundImage: 'url(' + headerImage + ')' }"></div>
      <div class="modal__content">
        <div class="modal__header" :class="{'-centered': headerCentered }">
          <wnl-icon-button
              touchable
              :icon="ICONS.FA_XMARK"
              class="modal__close"
          ...
          />
          <feature-icon v-if="headerFeatureIcon" class="modal__headerFeatureIcon" :color="calcHeaderFeatureIconColor" :icon="headerFeatureIcon" :size="FEATURE_ICON_SIZES.X_LARGE" :double-background="true" />
          <h3 class="modal__headerTitle" :class="{ '-small': headerTitleSize === MODAL_HEADER_TITLE_SIZES.SMALL }">{{ headerTitle }}</h3>
          <h6 v-if="headerSubtitle" class="modal__headerSubtitle">{{ headerSubtitle }}</h6>
        </div>
        <div v-if="$slots.default" class="modal__textContent">
          <slot />
        </div>
        <div class="modal__footer" :class="{ '-singleColumn': calcSingleColumn }">
          <div class="modal__footerColumn --ctaSecondary" v-if="footerTertiaryButtonText || footerCheckboxText">
            <div
                v-if="footerCheckboxText"
                ref="button"
            ...
            <wnl-button
                ...
                :iconLeft="ICONS[footerTertiaryButtonIcon]"
            >
              {{ footerTertiaryButtonText }}
            </wnl-button>
          </div>
          <div class="modal__footerColumn --cta" v-if="footerSecondaryButtonText || footerPrimaryButtonText">
            <wnl-button
                ...
                :iconRight="ICONS[footerSecondaryButtonIcon]"
            >
            ...
            <wnl-button
                ...
                :iconRight="ICONS[footerPrimaryButtonIcon]"
            >
...
export default {
  name: 'Modal',
  components: { FeatureIcon, WnlIcon, WnlButton, WnlIconButton },
  props: {
    size: {
      type: String,
