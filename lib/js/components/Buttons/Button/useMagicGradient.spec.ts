...
describe('useMagicGradient', () => {
	beforeEach(() => {
		document.getElementById(MAGIC_GRADIENT_ID)?.remove();
	});
	afterEach(() => {
		document.getElementById(MAGIC_GRADIENT_ID)?.remove();
	});
	...
	it('injects a single linearGradient with the hardcoded id when enabled', () => {
		...
		const gradient = document.getElementById(MAGIC_GRADIENT_ID);
	...
	it('does not inject the def when disabled', () => {
		...
		expect(document.getElementById(MAGIC_GRADIENT_ID)).toBeNull();
	});
	...
	it('injects the def when the reactive source becomes enabled', async () => {
		...
		expect(document.getElementById(MAGIC_GRADIENT_ID)).not.toBeNull();
	});
	...
	it('is idempotent — multiple enabled consumers leave exactly one def', () => {
		...
		expect(document.querySelectorAll(`#${MAGIC_GRADIENT_ID}`)).toHaveLength(1);
	});
});
