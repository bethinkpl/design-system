...
describe('Button', () => {
	...
	test.each([
		...
		[{ props: { color: BUTTON_COLORS.NEUTRAL }, expectedClass: '-color-neutral' }],
		[{ props: { color: BUTTON_COLORS.DANGER }, expectedClass: '-color-danger' }],
		[{ props: { color: BUTTON_COLORS.INVERTED }, expectedClass: '-color-inverted' }],
	...
	it('should render icon', () => {
		const leftIcon = createComponent({ 'icon-left': ICONS.FA_TIMES_LIGHT });
		expect(leftIcon.find('.a-button__icon.-left').exists()).toBe(true);
		expect(leftIcon.find('.a-button__icon.-left').props().icon).toBe(ICONS.FA_TIMES_LIGHT);
