...
<Story name='interactive'
		...
		argTypes={{
			...
			expandedContent: {control: { type: 'text' }, defaultValue: 'Dodatkowa treść w slocie <code>expandedContent</code> widoczna po rozwinięciu'},
		}}
		args={{
	...
	{(args) => {
		return {
			...
			template: '<card-expandable :is-expander-visible="isExpanderVisible" :expander-text-collapsed="expanderTextCollapsed" :expander-text-expanded="expanderTextExpanded" :is-expanded="isExpanded">' +
