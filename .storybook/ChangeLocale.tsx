...
import { useStorybookApi, useChannel, useGlobals } from '@storybook/manager-api';
...
export const ChangeLocale = memo(function MyAddonSelector() {
	...
	const [locale, setLocale] = useState(globals.locale || SUPPORTED_LOCALE.pl);

	const api = useStorybookApi();

	const changeLocale = useCallback(() => {
		const newLocale =
			locale === SUPPORTED_LOCALE.pl ? SUPPORTED_LOCALE.en : SUPPORTED_LOCALE.pl;
		...
		// Force remount the current story to apply locale changes
		emit(FORCE_REMOUNT, { storyId: api.getUrlState().storyId });
	}, [locale, emit, api, updateGlobals]);
